...
```md definition-list
`--userId [userId]`
: ID of the user whose calendar groups to retrieve. Specify either `userId` or `userName`, but not both. When using delegated permissions for a different user, requires the `Calendars.Read.Shared` or `Calendars.ReadWrite.Shared` scope.

`--userName [userName]`
: UPN of the user whose calendar groups to retrieve. Specify either `userId` or `userName`, but not both. When using delegated permissions for a different user, requires the `Calendars.Read.Shared` or `Calendars.ReadWrite.Shared` scope.
```
